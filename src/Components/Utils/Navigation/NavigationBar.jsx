...
import { LocalLibrary, Settings, Extension, Home, SmartToy, People, PersonSearch, LegendToggle} from '@mui/icons-material';
...
function NavigationBar() {
  ...
  return (
    <>
      <SideNavTrigger onClick={toggleSideNav} />
      
      <Box className={`side-nav-container ${isSideNavOpen ? 'open' : ''}`}>
        <Box className="side-nav-background">
          <Box className="side-nav-buttons">
            <Link to="/" className="nav-link">
              <Button 
                className={`nav-button side-nav-button ${activeButton === 'home' ? 'active' : ''}`}
                onClick={() => setActiveButton('home')}
              >
                <Home sx={{ mr: 1 }} /> 
                Home
            ...
            <Link to="/learn" className="nav-link">
              <Button 
                className={`nav-button side-nav-button ${activeButton === 'learn' ? 'active' : ''}`}
                onClick={() => setActiveButton('learn')}
              >
                <LocalLibrary sx={{ mr: 1 }} /> 
                Learn
            ...
            <Link to="/famous-games" className="nav-link">
              <Button 
                className={`nav-button side-nav-button ${activeButton === 'famous-games' ? 'active' : ''}`}
                onClick={() => setActiveButton('legends')}
              >
                <LegendToggle sx={{ mr: 1 }} />
                Legends
      ...
      >
        <DialogContent className='nav-dialog-content'>
          <Link to='/PlayWithComputer' onClick={handleClose} className='ButtonLink'>
            <Button className="nav-dialog-button" startIcon={<SmartToy />}>
              PLAY With COMPUTER
            </Button>
          </Link>
        </DialogContent>
